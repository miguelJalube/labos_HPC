    ...
    int index = 0;
        ...
        for(int i = 0; i < img->height; ++i){
            ...
            for(int j = 0; j < img->width; ++j){
                ...
                result->data[j + pos] = r*FACTOR_R + g*FACTOR_G + b*FACTOR_B;
                index += img->components;
            }
        }
    #else
        __m128i v1, v2, v3, res;
        v3 = _mm_set_epi32(0,0,0,0);
        for(int i = 0; i < img->height; ++i){
            int pos = i * img->width;
            for(int j = 0; j < img->width; ++j){
                // Set des vectteurs
                v1 = _mm_set_epi32(img->data[index + R_OFFSET], img->data[index + G_OFFSET], img->data[index + B_OFFSET], 0);
                v2 = _mm_set_epi32(FACTOR_R, FACTOR_G, FACTOR_B, 0);

                // Multiplcation des facteurs
                v1 = _mm_mul_epi32(v1, v2);

                // Addition horizontale
                res = _mm_hadd_epi32(v1, v3);

                // Stockage du résultat
                _mm_storeu_si32(&result->data[j + pos], res);
                index += img->components;
            }
        }
    #endif
